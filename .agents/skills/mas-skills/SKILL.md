...
8. Every PR adds exactly one concise entry to the top (unreleased) version in `CHANGELOG.md`, then runs `python scripts/changelog.py sync`; never edit generated version files by hand. Put contributor-only changes under `开发流程`, and user-visible changes under the matching category. Do not create a commit unless the user explicitly asks for one.
...
8. Every PR includes exactly one matching `CHANGELOG.md` entry under the top (unreleased) version, with generated version files synchronized.
