...
- 用户可见的功能或问题修复必须在提交pr时补充 `CHANGELOG.md`；把条目写进文件顶部第一个版本段（即当前未发布版本）匹配的分类下，**一条 PR 只贡献一条 changelog 记录，并必须用一句最简洁的语言概括该 PR 的意义**，将全部改动合并为一句话。改完运行 `python scripts/changelog.py sync` 同步 `res/version.json` 与各处版本号，二者一起提交；不要手改生成物，也不要手写 ` by [@用户]` 署名。
- `CHANGELOG.md` 遵循 [Keep a Changelog](https://keepachangelog.com/zh-CN/1.1.0/)，分类用 `新增` / `变更` / `弃用` / `移除` / `修复` / `安全` 六类；只影响贡献者的改动写进 `开发流程`。
- 破坏性的、或需要用户动手确认的改动写进置顶的 `破坏性变更` 分类；`本次亮点` 由维护者在发版前挑选，贡献者一般不用动。
