...
- 每个 PR 必须在 `CHANGELOG.md` 顶部第一个版本段（即当前未发布版本）新增且仅新增一条记录，并用一句最简洁的语言概括该 PR 的意义。用户可见的功能或问题修复写进匹配的分类；只影响贡献者的改动写进 `开发流程`。改完运行 `python scripts/changelog.py sync` 同步 `res/version.json` 与各处版本号并一起提交；不要手改生成物，也不要手写 ` by [@用户]` 署名。
