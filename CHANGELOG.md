...
- MFW 新增「托管」脚本类型（小范围试用，暂不默认开放）：由 AUTO-MAS 导入并精简项目，去掉界面程序、自带 Python 与 MaaFramework，多个脚本共用同一套运行环境；导入后可查看精简报告（省下多少、移除了哪些外壳、逐条排除原因）并随时切换或删除版本，更新时下载整包作为新版本导入、旧版本仍可切回，现有的 MFW 脚本也能一键转为托管（可选删除原项目目录）
