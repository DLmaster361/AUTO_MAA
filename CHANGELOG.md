...
- 绝区零一条龙专项 用户卡片新增「用户模式/直控模式」来源、区服、账号尾号与一条龙任务数标签，并修复 MaaFW 专项用户卡片不显示任何标签的问题
- MFW专项 使用模拟器时可顺带把游戏一起打开（包名默认从项目里自动识别，识别不出或识别到多个时可在脚本管理页手动填写），任务前后的自定义脚本改为每用户各跑一次、不再随重试重复执行

### 变更

- 全局设置 虚拟显示器改为常驻监测：MAS 运行期间只要检测不到任何真实显示输出就自动挂上虚拟显示器，真实显示器一恢复就自动拆掉（任务正在跑时等这轮任务结束再拆），不再随任务开始与结束插拔 by [@qiyinxi](https://github.com/qiyinxi)
...
- 修复后端在就绪前退出时只能反复重试的问题，现在会同时给出「重建运行环境」的修复入口 by [@qiyinxi](https://github.com/qiyinxi)
- MAA专项 修复后端未返回分服关卡数据时用户配置页整页打不开的问题 by [@qiyinxi](https://github.com/qiyinxi)
- 模拟器管理 修复部分 MuMu 版本因命令输出混有日志而无法读取模拟器信息的问题 by [@qiyinxi](https://github.com/qiyinxi)
