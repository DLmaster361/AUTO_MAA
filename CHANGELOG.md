...
- 模拟器管理 新增「Emulator 2.0」：一条配置纳管多个雷电 14 / MuMu 6 安装路径与实例，支持新建/删除实例、分辨率/CPU/内存/帧率单台与批量设置、一键稳定模式与配置守卫 by [@qiyinxi](https://github.com/qiyinxi)
- 新增绝区零一条龙（ZZZ-OD）专项，支持独立模式与直控模式 by [@AthenaHibou](https://github.com/AthenaHibou)
- 调度队列 新增循环队列：每个任务可单独设定固定时间或间隔重复运行 by [@qiyinxi](https://github.com/qiyinxi)
- MaaEnd专项 新增脚本直控模式，抢委托送货与自动采集独立为可配置阶段，支持每日仅执行一次 by [@HarcoChen](https://github.com/HarcoChen)
- MFW 项目支持运行前/后自动更新，可选下载源（Mirror 酱/GitHub）与更新通道（稳定/测试版） by [@qiyinxi](https://github.com/qiyinxi)
- MAA专项 新增绿票商店：每月单独启动一次自动购买，用户页可查看状态并重置 by [@qiyinxi](https://github.com/qiyinxi)
- OK-NTE专项 支持按手机号后 4 位切换账号、由 MAS 拉起异环游戏，新增问题包导出 by [@AthenaHibou](https://github.com/AthenaHibou)
- 调度中心 选中脚本后可指定单个用户运行；队列完成后操作可单独设定延时 by [@qiyinxi](https://github.com/qiyinxi)
- 通知系统 支持微信 Claw 和 QQ 官方机器人接收任务通知 by [@HarcoChen](https://github.com/HarcoChen)
- HSR专项 用户配置新增「额外脚本」，任务前后各执行一个自定义脚本，管控与直控均生效 by [@qiyinxi](https://github.com/qiyinxi)
- 首页 快速启动支持多选并记住上次选择 by [@Craun718](https://github.com/Craun718)
- MAA专项 托管结束后保存每日状态，同天多次托管不重复执行 by [@1w1w11w1](https://github.com/1w1w11w1)
- 全局设置 新增虚拟显示器：检测不到真实显示输出时临时挂 1920×1080 虚拟屏，任务结束即拆除，可选 60/30Hz，需自行安装 Parsec 驱动 by [@qiyinxi](https://github.com/qiyinxi)
- 更新提示改为按版本分区块展示，重要变更与亮点置顶，设置页可随时查看当前版本日志 by [@qiyinxi](https://github.com/qiyinxi)
...
- Emulator 2.0 修复模拟器已运行时不开游戏、雷电 0 号与 MuMu 同时运行时操作串台的问题 by [@qiyinxi](https://github.com/qiyinxi)
- 日志采集 修复跨零点日志滚动后运行历史与节点详情丢失的问题 by [@AthenaHibou](https://github.com/AthenaHibou)
- 签到 结果统一附加到任务报告并按渠道展示，修复重复推送与开启通知时必报 TypeError 的问题 by [@Lance0174](https://github.com/Lance0174) by [@1w1w11w1](https://github.com/1w1w11w1)
- Runtime 修复首次初始化时「安装 Python」进度倒退的问题 by [@qiyinxi](https://github.com/qiyinxi)
- 启动界面 修复「查看日志」窗口空白、失败时无日志入口、说明贴顶、日志打不开无提示等问题 by [@qiyinxi](https://github.com/qiyinxi)
- 修复深色模式下初始化界面连同标题栏变暗的问题 by [@ClozyA](https://github.com/ClozyA)
- 修复两个前端窗口互抢连接反复断线的问题；断开连接改为非阻塞提示、重连后自动收起；更新后端期间的断开按计划内处理不再弹窗 by [@qiyinxi](https://github.com/qiyinxi)
- 修复渲染进程崩溃后窗口永久黑屏的问题 by [@qiyinxi](https://github.com/qiyinxi)
- 首页 修复终末地活动缓存日期异常导致黑屏、活动卡片悬停边缘反复抖动的问题 by [@1w1w11w1](https://github.com/1w1w11w1) by [@Shasnow](https://github.com/Shasnow)
- MAA专项 修复日常理智误用剿灭队列、剿灭因体力耗尽被误判完成额度、关闭理智作战后活动关消失、v6.14 后不再按配置切号的问题 by [@1w1w11w1](https://github.com/1w1w11w1) by [@qiyinxi](https://github.com/qiyinxi)
- MAA专项 修复 MAA 卡死后不会超时的问题：日志停滞提示反复重置计时，现识别新旧两版 MAA 五种语言的该提示 by [@qiyinxi](https://github.com/qiyinxi)
- MFW专项 修复运行环境损坏/不可用时到任务中途才报错的问题，环境不可用时不再反复重启模拟器白等；便携版装有另一份 Python 3.12 时不再报 ctypes 错误；建项向导在环境就绪前不允许下一步 by [@qiyinxi](https://github.com/qiyinxi)
- MFW专项 受限网络新增 Python 镜像环境变量；回退旧链路时复用 Runtime 已装的 uv；找不到 uv 时给出可照做的中文提示 by [@qiyinxi](https://github.com/qiyinxi)
- MFW专项 项目更新改为从 interface.json 自动识别，不再需手填 GitHub 信息；版本检查不再占用 Mirror 酱下载次数；CDK 过期/不符/用尽时说明原因 by [@qiyinxi](https://github.com/qiyinxi)
- MFW专项 修复 interface 预设重复写同一任务时整份项目读不出来、脚本强制停止被记成完成的问题 by [@qiyinxi](https://github.com/qiyinxi)
- HSR专项 修复文本/JSON 配置项输入时被清空、培养目标导致历战余响不刷、三月七关游戏导致模块判失败、SRA 升级后覆盖配置让全部表单消失、模块重试耗尽仍标为完成、直控偶发标异常等问题 by [@qiyinxi](https://github.com/qiyinxi)
- HSR专项 修复差分宇宙/货币战争统计与周常标签不一致、体力补充方式选项错位一格、调度台时间戳硬编码东八区的问题 by [@qiyinxi](https://github.com/qiyinxi)
- HSR与SRC专项 修复英文/日文界面术语错误（历战余响被显示为差分宇宙等） by [@qiyinxi](https://github.com/qiyinxi)
- OK-WW与OK-NTE专项 修复多用户切换时误复用正退出的旧窗口、账号切换定位不到窗口或界面未就绪即误操作、未回到登录界面时误报就绪的问题 by [@AthenaHibou](https://github.com/AthenaHibou)
- OK-NTE专项 加固切号流程预防弹窗/屏保；修复体力不足时报告误显剩余 0、日常已领取时误判失败反复重试的问题 by [@AthenaHibou](https://github.com/AthenaHibou)
