...
本项目采用自主开发的混合加密模式，项目组也无法找回您的管理密钥或修复data/key/目录下的文件。如果不幸的事发生，建议您删除data/key/目录与data/data.db文件后重新录入信息。
```
...
你可以自定义关卡号替换方案。程序会读取data/gameid.txt中的数据，依据此进行关卡号的替换，便于常用关卡的使用。gameid.txt在初始已经存储了一些常用资源本的替代方案。
```
