...
def backup_root(script_id: str) -> Path:
    """某脚本的备份归档根目录：``data/{script_id}/OkNteBackups``。"""

    return Path.cwd() / "data" / script_id / "OkNteBackups"


def mas_backup_root(script_id: str, user_id: str) -> Path:
    """MAS 用户配置的归档目录：``.../OkNteBackups/mas/{user_id}``。"""

    return backup_root(script_id) / "mas" / user_id


def native_backup_root(script_id: str) -> Path:
    """脚本原生配置的归档目录：``.../OkNteBackups/native``。"""

    return backup_root(script_id) / "native"
...
def archive_native_backup(
    ...
    if not files:
        return None
    dest = archive_files(files, native_backup_root(script_id), force=force)
    if dest is None:
...
def list_native_backups(script_id: str) -> list[str]:
    """ok-nte 原生配置全部归档时间戳（倒序，最新在前）。"""

    return list_times(native_backup_root(script_id))


def get_native_backup_dir(script_id: str, ts: str) -> Path | None:
    """取指定时间戳的原生配置归档目录；不存在返回 None。"""

    return get_backup_dir(native_backup_root(script_id), ts)


def restore_native_backup(
    script_id: str, ts: str, config_path: Path, mode: str
) -> None:
    ...
    backup_dir = get_native_backup_dir(script_id, ts)
    if backup_dir is None:
        raise ValueError(f"备份不存在: {ts}")
    config_path = Path(config_path)
    # 恢复前强制归档当前——「恢复前的配置」在列表里有明确的时间戳条目
    archive_native_backup(script_id, config_path, mode, force=True)
    if mode == "File":
