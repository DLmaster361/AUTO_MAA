...
export default {
  ...
  setting: {
    ...
    display: {
      section: '仮想ディスプレイ',
      intro:
        'モニターを外したり電源を切ったりすると Windows はデスクトップを非常に低い解像度へ退避します。管理対象の PC 版ゲームのウィンドウは縮んだままゲームに記憶され、以降の無人実行が常に条件を満たさなくなります。有効にすると、MAS は「ゲームウィンドウを収められる画面が一つも無い」場合にだけ仮想ディスプレイを一時的に接続し、タスク終了時に取り外します。Parsec 仮想ディスプレイドライバーは各自でインストールしてください。MAS には同梱されていません。',
      enable: '仮想ディスプレイを有効化',
      enableTip:
        'デスクトップが実際に狭いときだけ接続します。モニターが正常なら追加しません。MAS の終了後、仮想ディスプレイは自動的に消えます。',
      mode: 'リフレッシュレート',
      modeTip:
