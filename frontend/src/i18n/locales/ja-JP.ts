...
export default {
  ...
  edit: {
    ...
    bettergiMasConfigTooltip: 'ユーザー独立設定：このユーザーの一条龍タスクとカスタム設定グループはすべてこのページ（MAS 側）で設定します。BetterGI の「一条龍」ページを編集する必要はありません。',
    ...
    bettergiDirectModeAlert: '「スクリプト直接制御」モード：下欄でこのユーザーが使う一条龍名（BetterGI に存在する設定名）を入力してください。スクリプトの設定は BetterGI 内で行います（「BetterGI を設定」をクリックして開けます）。',
    bettergiSwitchToMasConfig: 'ユーザー独立設定に切り替える',
    bettergiMasConfigHowTo: '「ユーザー独立設定」の使い方',
    bettergiMasConfigHowTo1a: 'このユーザーの一条龍は独立設定で動作し、タスクとカスタム設定グループはこのページ（MAS 側）で設定します（BetterGI の「一条龍」ページを開く必要はありません）。MAS は固定スロット',
    bettergiMasConfigSlotName: '「MAS独立配置」',
    bettergiMasConfigHowTo1b: 'から一条龍を起動し、終了後にスロットを自動クリーンアップします。既存の BetterGI 設定（「默认配置」など）には一切触れません——同名の実設定は読み込まれず、ここでの編集の影響も受けません。',
    ...
    bettergiOneDragonNameHint: 'ユーザー独立設定がオンの間は「MAS独立配置」に固定され変更できません。オフ（直接制御モード）では使用する BetterGI 設定を選択します。既定は「默认配置」です',
    ...
    bettergiStrategyPickerTip: '上の候補をクリックすると選択されます（単一選択）。空欄の場合は「パーティーに応じて自動選択」になります',
    ...
    bettergiGroupStamina: 'スタミナ作戦',
    bettergiGroupKindStamina: '専項',
    bettergiGroupKindCustom: 'スクリプト',
    bettergiGroupKindJs: 'JS',
    bettergiGroupKindJsDesc: 'BetterGI のスクリプトディレクトリ User/JsScript にあるカスタム JS スクリプト。それぞれ一条龍の独立タスクとして実行できます（フォルダ名がタスク名）。追加後は AUTO-MAS が設定グループとしてスイッチ管理します。',
    bettergiGroupsDragHint: '左のハンドルをドラッグすると設定グループの並び順を変更できます（順序はバックエンド連携後に有効になります）。',
    bettergiGroupStaminaHint: 'スタミナ作戦は AUTO-MAS モジュールが追加した仮想設定グループ（9 番目）です。週間スケジュールに従って樹脂を賢く消費します（地脈・秘境・ボス討伐）。このスイッチは現在フロントエンドのみの予約で、バックエンド連携後に一条龍実行へ反映されます。',
    bettergiGroupsEmptyRight: '左側で設定グループを選択すると、右側に説明とスイッチが表示されます。',
    bettergiGroupPrefixDefault: 'デフォルト',
    bettergiGroupFrozen: '凍結',
    bettergiGroupFrozenTip: 'スタミナ作戦が有効なため、刷取系の組み込みグループは一時的に凍結されています。スタミナ作戦を無効化または一条龍から外すと再び有効化できます。',
    bettergiAddToDragon: '設定グループを追加',
    bettergiAddToDragonOk: '一条龍に追加',
    bettergiPickCandidateFirst: '下の候補グループをクリックしてください',
    bettergiAlreadyInDragon: 'このグループは既に一条龍に追加されています',
    bettergiInQueue: 'キュー内',
    bettergiDuplicateAsNew: '新しい設定グループとして保存',
    bettergiCopySameAs: '同じ設定グループを複製',
    ...
    bettergiDuplicateNameTaken: '「{name}」は現在の一条龍に既に存在します。別の名前にしてください',
    bettergiDuplicateDone: '新しい設定グループ「{name}」として保存し、キューに追加しました',
    bettergiDuplicateSameDone: '設定グループ「{name}」を複製してキューに追加しました',
    bettergiRemoveFromDragonTitle: '一条龍から外す',
    bettergiRemoveFromDragonContent: '「{name}」を一条龍から外しますか？後で「設定グループを追加」から再追加できます。',
    bettergiClearDragon: '全てクリア',
    bettergiClearDragonTitle: '一条龍をクリア',
    bettergiClearDragonContent: '一条龍全体をクリアしますか？すべての設定グループが外され、後で手動で再追加できます。',
    bettergiInputGroupNames: '設定グループ名を入力して Enter、または下の候補をクリック',
    bettergiInputGroupNamesTip: 'バブル＝選択済みグループ（タイプ別の色）。× で削除。候補/パスファイルのクリックでバブル追加。手入力は Enter または「;」で確定（未認識の名前はブロック）。',
    bettergiGroupNamesUnknown: '認識できない設定グループ：{names}',
    ...
    bettergiCustomGroupsTip3: '「設定グループを追加」では、既存の BetterGI 設定（独立設定モードではこのユーザーの MAS 独立コピー）から管理対象のグループを選びます。表に入れていないグループも一条龍に残り、この表が原因で失われることはありません。',
    ...
    bettergiOpenScriptRepo: 'スクリプトリポジトリを開く',
    bettergiOpenScriptDir: 'スクリプトフォルダを開く',
    bettergiOpenTaskDir: 'AutoPathing フォルダを開く',
    bettergiOpenOneDragonDir: '一条龍フォルダを開く',
    bettergiOpenScriptGroupDir: '設定グループフォルダを開く',
    bettergiOpenBgi: 'BGI を開く',
    bettergiMultiSelectHint: 'Shift クリックで範囲選択 / Ctrl クリックで個別選択',
    bettergiMultiRemove: '選択を削除',
    bettergiMultiEnable: 'すべて有効',
    bettergiMultiDisable: 'すべて無効',
    bettergiMultiSelected: '{count} 件選択中',
    bettergiChipSelectHint: 'クリックで選択 / ドラッグで連続選択、Ctrl で個別、Shift で範囲、Backspace または Delete で選択チップを削除',
    bettergiProjectUnselect: '選択解除',
    bettergiMultiRemoveTitle: '選択した設定グループを削除',
    bettergiMultiRemoveContent: '選択した {count} 件の設定グループを削除しますか？',
    bettergiTabScriptGroup: '設定グループ',
    bettergiTabJsScript: 'JSスクリプト',
    bettergiTabPathing: 'マップ追跡',
    bettergiScriptGroupEmptyDir: 'User/ScriptGroup に設定グループがありません。「設定グループフォルダを開く」から追加できます',
    bettergiTabJsEmpty: 'カスタム JS スクリプトやカスタムグループはまだありません。上部の「スクリプトフォルダを開く」から確認できます',
    bettergiGroupKindScriptGroup: '設定グループ',
    bettergiGroupKindScriptGroupDesc: 'BetterGI の設定グループフォルダ User/ScriptGroup にあるカスタム設定グループ。一条龍に直接追加できます。',
    bettergiPathingSelectHint: '左側でフォルダを選択し、右側のファイルをクリックすると「パス」バブルが追加されます。「一条龍に追加」で確定します。',
    bettergiPathingEmptyDir: 'このフォルダにマップファイルはありません',
    bettergiPathingEmptyTree: 'AutoPathing フォルダが見つかりません。BGI スクリプトのパスを確認してください',
    bettergiGroupKindPathing: '経路',
    bettergiGroupKindPathingDesc: 'BetterGI のマップ追跡フォルダ User/AutoPathing にある経路ファイル。一条龍に追加して採集・討伐ルートを実行できます。',
    ...
    bettergiGroupSettingsTitle: 'タスク設定',
    bettergiDomainPickerTitle: '秘境を選択（地域 → 秘境 → 報酬）',
    bettergiDomainPickerNone: '利用可能な設定項目はありません',
    bettergiDomainPickerClear: 'クリア',
    bettergiDomainPickerOk: '確定',
    bettergiDomainPickerClearAll: 'クリア',
    bettergiDomainPickerClearAllConfirm: 'この表のすべての行の秘境と報酬をクリアしますか？',
    bettergiGroupSettingsSave: '設定を保存',
    bettergiGroupSettingsPlaceholder: '選択または入力',
    bettergiGroupSettingsNone: 'このタスクに設定項目はありません',
    bettergiGroupSettingsZoom: 'ダイアログで設定を表示・編集',
    bettergiGroupSettingsLoadFailed: 'タスク設定の読み込みに失敗しました',
    bettergiGroupSettingsSaveFailed: 'タスク設定の保存に失敗しました',
    bettergiGroupSettingsSaved: 'タスク設定を保存しました',
    bettergiProjectAddScript: 'スクリプトを追加',
    bettergiProjectRemoveScript: 'スクリプトを削除',
    bettergiProjectRemoveConfirm: '選択したスクリプトプロジェクトを削除しますか？',
    bettergiProjectToolbarTip: '左 2/3 エリアをドラッグして並べ替え。クリックで選択、Ctrl で複数選択、Shift で範囲選択',
    bettergiProjectZoom: '拡大して編集',
    bettergiProjectRowTip: 'クリックで選択、Ctrl で複数切替、Shift で範囲選択、ダブルクリックでスクリプト設定を開く',
    bettergiProjectMultiHint: '複数選択中 - 「スクリプトを削除」で一括削除できます',
    bettergiProjectClearSelection: '選択を解除',
    bettergiAddScriptToGroup: 'スクリプトを設定グループへ追加',
    bettergiAddScriptToGroupOk: '設定グループへ追加',
    bettergiAddScriptUnsupported: '選択した項目はスクリプトとして追加できません（JS スクリプトとマップパス追跡のみ対応）',
    bettergiProjectListTip: '左 2/3 エリアをドラッグしてプロジェクトの順序を変更できます。ダブルクリックで「スクリプト設定 / スクリプト説明」を開きます。JS/経路は単一プロジェクトのグループとして表示されます',
    bettergiProjectEmpty: 'この設定グループにプロジェクトはありません',
    bettergiProjectDblclickEdit: 'ダブルクリックでスクリプト設定と説明を開く',
    bettergiProjectSettingsSave: '保存',
    bettergiProjectSettingsDblclickTip: '変更はこのユーザー設定に保存されます（BetterGI の元設定は上書きされません）',
    bettergiProjectNoSettingsFile: 'このスクリプトフォルダには設定ファイル（settings.json）がなく、編集項目はありません',
    bettergiProjectConfigTab: 'スクリプト設定',
    bettergiProjectReadmeTab: 'スクリプト説明',
    bettergiProjectReadmeEmpty: 'このスクリプトフォルダには説明ドキュメント（README）がありません',
    bettergiProjectLoadFailed: '設定グループのプロジェクト読み込みに失敗しました',
    bettergiProjectSaveFailed: '設定グループのプロジェクト保存に失敗しました',
    bettergiProjectSaved: '設定グループのプロジェクトをユーザー設定に保存しました',
    bettergiProjectIndependentSaveLater: '独立 JS/経路の設定保存は今後のバージョンでサポート予定です',
  },
  emulator: {
    title: 'エミュレータ管理',
    type: {
