...
def print_table(data):
    # 创建一个PrettyTable对象并指定列名
    table = PrettyTable()
    table.field_names = ["用户名", "手机号码", "代理天数", "状态", "执行情况", "关卡",
                         "密码", "备选关卡1", "备选关卡2", 'QQ号']

    # 确保数据的每一行都有正确数量的元素（9个元素，对应9列）
    for row in data:
        if len(row) == 10:
            # 添加行到表格
            table.add_row([
            ...
            print(f"Error: Row has incorrect number of elements: {row}")
...
def getPASSWORD(PASSWORD):
    try:
        ...
        # 生成密钥转换与校验随机盐，固定长度为32字节
        PASSWORDsalt = secrets.token_hex(16)
        verifysalt = secrets.token_hex(16)
        ...
        # 使用CBC模式加密RSA私钥并保存密文，需要IV
        iv = secrets.token_bytes(16)  # 生成16字节的IV
        AES_key = AES.new(AES_password, AES.MODE_CBC, iv)
        private_key_padded = pad(private_key.exportKey(), AES.block_size)
        private_key_encrypted = AES_key.encrypt(private_key_padded)

        # 保存IV和加密后的私钥
        with open("data/key/private_key.bin", "wb") as f:
            f.write(iv + private_key_encrypted)  # 将IV和加密后的私钥一起保存
...
def decryptx(note, PASSWORD):
    ...
    if AES_password_SHA != AES_password_verify:
        raise ValueError("管理密钥错误")

    # AES解密RSA私钥
    AES_key = AES.new(AES_password, AES.MODE_CBC, iv)
    private_key_pem = unpad(AES_key.decrypt(private_key_encrypted), AES.block_size)
...
def add():
    ...
    dayx = int(input("代理天数："))
    gamex = input("关卡号：")
    spare_game = input("备用关卡1：")
    spare_game_2 = input('备用关卡2:')
    qq_id = input('请输入QQ号：')
    ...
    db = sqlite3.connect(DATABASE)
    cur = db.cursor()
    cur.execute("INSERT INTO adminx VALUES(?,?,?,'y','2000-01-01',?,?,?,?,?)",
                (adminx, numberx, dayx, gamex, passwordx, spare_game, spare_game_2, qq_id))
    db.commit()
    cur.close()
    db.close()
    return "操作成功"
...
def search(id, book):
    ...
    if len(data) > 0:
        ...
        if book == 1:
            print('')
            print_table(data)
            # print(unit("用户名", 15), unit("手机号码", 12), unit("代理天数", 8), unit("状态", 4), unit("执行情况", 10),
            #       unit("关卡", 10), unit("密码", 25), unit('备选1', 5), unit('备选2', 5))
            # for i in range(len(data)):
            #     print(unit(data[i][0], 15), unit(data[i][1], 12), unit(data[i][2], 8), unit(data[i][3], 4),
            #           unit(data[i][4], 10), unit(data[i][5], 10), unit(data[i][6], 25),
            #           unit(data[i][7], 4), unit(data[i][8], 4))
        return ""
    elif id == "all":
        return "\n当前没有用户记录"
    else:
...
def gameid(readxx):
    # 提取用户名与修改值
    id, gamep, spare_game, spare_game2 = "", "", "", ""
    parts = readxx.split()
    if len(parts) >= 3:
        id = parts[0]
        gamep = parts[1]
    ...
    # 更新主关卡规则
    gamep = games.get(gamep, gamep)
    # 更新备选关卡规则
    spare_game = games.get(spare_game, spare_game)
    # 更新备选关卡2规则
    spare_game2 = games.get(spare_game2, spare_game2)

    # 应用更新
    db = sqlite3.connect(DATABASE)
    cur = db.cursor()
    cur.execute("UPDATE adminx SET game=?, spare_game=?, spare_game_2=? WHERE admin=?",
                (gamep, spare_game, spare_game2, id))
    db.commit()
    cur.close()
    db.close()
    return f'成功更新{id}的关卡为{gamep}, 备选关卡为{spare_game}, 备选关卡2为{spare_game2}'
...
if not os.path.exists(DATABASE):
    db = sqlite3.connect(DATABASE)
    cur = db.cursor()
    db.execute("CREATE TABLE adminx(admin text,number text,day int,status text,last date,game text,password byte,spare_game text,spare_game_2 text,qq_id text)")
...
while True:
    ...
    if len(read) == 0:
        ...
        if read[-1] == '?' and read[-2] == ' ':
            ...
            print('请输入id，主关卡，备选关卡1，备选关卡2，注意这四项之间需要用空格隔开')
            print(gameid(read[:-2]))
